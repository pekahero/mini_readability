...
class ConvertToText:
    def __init__(self, bs_tag: bs4.Tag, format_rules: dict = None,
                 wrap_len: int = 80, title: str = None):
    ...
    def _replace_html_tags_with_text(self):
        for tag_to_format in self.rules:
            format_rule_for_tag = self.rules[tag_to_format]
            for child in self.soup.findAll(tag_to_format):
                format_rule_for_tag(child, self.soup)
